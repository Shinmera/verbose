...
(defun make-standard-global-controller ()
  (let ((pipeline (make-instance 'controller))
        (pipe (make-pipe)))
    (vector-push-extend (make-instance 'predicate-filter :predicate #'(lambda (message) (eql (level message) *repl-level*))) pipe)
    (vector-push-extend (make-instance 'category-tree-filter) pipe)
    (vector-push-extend (make-instance 'repl-faucet) pipe)
    (add-pipe pipeline pipe)))
