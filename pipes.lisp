...
(defclass term-faucet (repl-faucet)
  ())

(defmethod format-message ((faucet term-faucet) (message message))
  (flet ((f (color format &rest args)
    ...
    (f 7 "~a " (local-time:format-timestring NIL (timestamp message) :format *timestamp-format*))
    (f 8 "[")
